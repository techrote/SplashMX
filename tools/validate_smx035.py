...
def main() -> int:
    for path in (DOC, FIXTURES, MODEL, RESOLVER, BUNDLE, SYSTEM, TESTS):
    ...
    require([row.get("id") for row in fixtures] == [f"PPK-{n:03d}" for n in range(1,31)], "PPK fixture set must be contiguous PPK-001..PPK-030")
    ...
    for token in (
        ...
        "promote_definition", "package.version_conflict",
    ):
