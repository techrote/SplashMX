...
def _copy_locus(current: CanonicalProjectRevision, selected: CanonicalProjectRevision, locus: str) -> CanonicalProjectRevision:
    ...
    if ":" not in locus:
        raise PeopleError("people.unsupported_conflict_locus", "This conflict requires an authored edit before it can be acknowledged.")
...
class PeopleSession:
    ...
    def __init__(
        ...
        *,
        ...
        # Actor sequence is transport/history metadata, never semantic identity. Resume
        # after the highest locally stored sequence to avoid replay/collision on restart.
        row = self.store._db.execute("SELECT COALESCE(MAX(actor_seq),0) FROM receipts WHERE actor_id=?", (principal_id,)).fetchone()
        pending = self.store._db.execute("SELECT COALESCE(MAX(actor_seq),0) FROM pending WHERE actor_id=?", (principal_id,)).fetchone()
        ...
        self._unsynced_parent: TransactionId | None = None
    ...
    def author_revision_counter(self) -> int:
        values = []
        for (value,) in self.store._db.execute("SELECT revision_id FROM revisions WHERE revision_id LIKE 'author-%'"):
            text = str(value)
    ...
    def record_local(self, candidate: CanonicalProjectRevision, *, fault_hook=None) -> IngestResult | None:
        ...
        if candidate == current:
            ...
            self._unsynced_parent = None
            return None
        base = self._unsynced_base or current
        parent = self._unsynced_parent if self._unsynced_base is not None else self._head_transaction()
        self._actor_seq += 1
    ...
    def enqueue_relay(self, packet: RelayPacket, authenticator: RelayAuthenticator) -> int:
        if not self._relay_online:
            ...
            raise PeopleError("people.relay_offline", "Collaboration relay is offline; local authored work remains available.")
        # Authenticate before consuming bounded queue capacity. Relay authentication
        # proves sender envelope integrity only; store permission epochs still decide
        # whether the semantic transaction may materialize.
        ...
        if len(self._relay_queue) >= MAX_RELAY_QUEUE:
            ...
            raise PeopleError("people.relay_backpressure", "Collaboration input is busy; retry without discarding local work.")
    ...
    def drain_relay(self, *, limit: int = MAX_RELAY_QUEUE) -> tuple[IngestResult, ...]:
        if not isinstance(limit, int) or isinstance(limit, bool) or limit < 1 or limit > MAX_RELAY_QUEUE:
            raise PeopleError("people.invalid_drain_limit", "Relay drain limit is outside the bounded collaboration profile.")
    ...
    def resolve_conflict(self, conflict_id: str, choice: str) -> IngestResult:
        ...
        if conflict is None or conflict.resolved_by is not None:
            raise PeopleError("people.unknown_conflict", "That collaboration conflict is no longer unresolved.")
        current = self.head_project()
        if choice == "current":
            document = deepcopy(current.document)
            assets = deepcopy(dict(current.assets))
            ...
            raise PeopleError("people.invalid_resolution", "Choose current, alternative A or alternative B.")
        self._actor_seq += 1
        document = deepcopy(candidate.document)
        document.project_revision_id = ProjectRevisionId(
            "people-resolve-" + sha256(f"{conflict_id}\0{choice}\0{current.document.project_revision_id}\0{self._actor_seq}".encode()).hexdigest()
        )
        candidate = CanonicalProjectRevision(document, deepcopy(dict(candidate.assets)))
        validate_project_revision(candidate)
        parent = self._head_transaction()
        transaction = create_transaction(
            actor_id=self.principal_id,
            actor_seq=self._actor_seq,
            parents=() if parent is None else (parent,),
        ...
        if result.status != "resolution":
            raise PeopleError("people.resolution_not_materialized", "Conflict resolution was retained but not published as canonical state.")
        return result
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        for conflict in self.store.unresolved_conflicts():
            ...
            conflicts.append({
                ...
                "choices": choices,
                "alternatives": [
                    {"choice": "alternative-a", "project_revision_id": str(conflict.alternative_a.document.project_revision_id)},
                    {"choice": "alternative-b", "project_revision_id": str(conflict.alternative_b.document.project_revision_id)},
                ],
            })
        return {
            "plane": "collaboration",
            ...
            "presence": [
                {"principal_id": row.principal_id, "cursor": row.cursor, "selections": list(row.selections)}
