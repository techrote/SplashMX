...
async function readJson(response) {
  ...
  if (!response.ok || !payload.ok) {
    ...
    const failure = new Error(error.message || "That change could not be applied.");
    failure.code = error.code || "browser.action_failed";
    throw failure;
  }
  return payload;
}
...
async function act(action, data = {}, success = "Change applied.") {
  try {
...
function selectedOne() {
  ...
  if (selected.length !== 1) throw new Error("Select exactly one Thing for this action.");
  return selected[0];
}
...
function renderStage() {
  ...
  function card(thing, depth = 0) {
    ...
    ports.textContent = thing.ports.length ? thing.ports.map((port) => `${port.name} · ${port.kind}/${port.direction}`).join("  |  ") : "No ports";
    wrapper.append(ports);
    for (const child of state.canonical.things.filter((candidate) => candidate.parent_thing_id === thing.thing_id)) wrapper.append(card(child, depth + 1));
    return wrapper;
  }
...
function renderInspect() {
  ...
  if (!(state.diagnostics || []).length) log.textContent = "No diagnostics.";
}
...
function render() {
  ...
  $("#mode").textContent = playing ? "Play mode" : "Edit mode";
...
$("#import-form").addEventListener("submit", async (event) => {
  ...
  await act("importAsset", {
    content_base64: btoa(binary), source_name: file.name, media_type: file.type || String(form.get("media_kind")),
    media_semantics: { kind: String(form.get("media_kind")) }, provenance: { origin: String(form.get("provenance")) },
...
document.addEventListener("keydown", async (event) => {
  if ((event.ctrlKey || event.metaKey) && event.key === "Enter") { event.preventDefault(); if (!state.runtime || state.runtime.mode !== "play") await act("play", {}, "Playing transient runtime state."); }
  else if (event.key === "Escape" && state.runtime && state.runtime.mode === "play") { event.preventDefault(); await act("stop", {}, "Stopped. Authored project is unchanged."); }
  else if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "s") { event.preventDefault(); await act("save", {}, "Saved locally."); }
  else if (event.altKey && event.key.toLowerCase() === "i") { event.preventDefault(); await act("inspect", { open: !state.editor.inspect_open }, "Inspect toggled."); }
});
