...
// Trusted test relay for topology-equivalence evidence. Sender identity is
// assigned at the connection boundary; it is never taken from payload fields.
export function startRelay(port = 8877) {
  ...
  function fromAuthority(room, source, payload) {
    const kind = payload.type;
    if (kind === 'checkpoint') {
      // A checkpoint is a late-join/reconnect/migration baseline, not another
      // live-state stream. Store it and deliver it only when a new transport
      // binds, so it cannot bypass relevance/unload handling on live replicas.
      room.checkpoint = structuredClone(payload);
      return;
    }
    const recipients = [...room.members.values()].filter((m) => m.connId !== source.connId);
    for (const target of recipients) {
      let delayMs = 0;
      if (kind === 'state') delayMs = payload.state_seq === 1 ? 180 : payload.state_seq === 2 ? 30 : 70;
      setTimeout(() => forward(room, source, target, payload, { relay_delay_ms: delayMs }), delayMs);
      if (kind === 'event') {
        // Duplicate one reliable application event: logical dedupe is a runtime
        // semantic requirement even over a reliable transport.
  ...
  wss.on('connection', (ws, req) => {
    ...
    const member = {
      ...
      watchLifecycle: topology === 'peer' && requestedRole === 'client',
    };
    ...
    if (authority && authority.connId !== member.connId) {
      if (room.checkpoint) {
        forward(room, authority, member, structuredClone(room.checkpoint), { baseline_delivery: true });
      } else {
    ...
    ws.on('message', (data) => {
      ...
      if (room.authorityConnId === member.connId) {
        fromAuthority(room, member, payload);
      } else {
