...
A stable `AssetId` still selects one indivisible immutable canonical revision containing revision/content digest, source digest and source identity, exact source metadata, audio/media semantic metadata, provenance, licence/attribution and derivation lineage. Trust authenticates bytes; decoder output is target-private. Neither may synthesize canonical meaning by field-mixing competing revisions. Complete-revision validation remains explicit and derivative cache placement/eviction is non-semantic.
